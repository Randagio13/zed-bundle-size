...
import { parse, exportImported, type ImportInfo } from "./parse";
...
const builtinExternalPlugin = (esbuild: typeof import("esbuild")) => ({
...
async function bundle(
  ...
  const { rootPkgFile, rootPkg, modulePkgFile, modulePkg } =
    await findPkgs(modulePath, baseDir).catch(
      () => ({} as Awaited<ReturnType<typeof findPkgs>>)
    );

  const cacheKey = cacheOpt && rootPkg
  ...
  const workingDir = projectPkgFile
    ? path.dirname(projectPkgFile)
    : baseDir;
